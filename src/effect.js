...
export default class Effect {
  ...
  animate(material, userData, mouseCoords) {
    return (
      gsap
        ...
        .set(
          material.uniforms.uDirection,
          { value: userData.isBw ? 1.0 : -1.0 },
          0
        )
  ...
  resetMaterial(object) {
    ...
    gsap
      ...
      .fromTo(
        object.material.uniforms.uGrayscaleProgress,
        { value: 1 },
        { value: 0 },
        0
      )
      .to(
        object.material.uniforms.uRippleProgress,
        { keyframes: { value: [0, 1, 0] } },
        0
      );
  }
  ...
  onClick(e) {
    ...
    if (intersection) {
      ...
      if (
        this.activeObject &&
        intersection.object !== this.activeObject &&
        this.activeObject.userData.isBw
      ) {
        this.resetMaterial(this.activeObject);

        // Stops timeline if active
        if (this.activeObject.userData.tl?.isActive())
          this.activeObject.userData.tl.kill();
